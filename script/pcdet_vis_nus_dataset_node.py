...
# -------------------------- config -------------------------- #
# data_root, classes = "/datasets/nuScenes/Fulldatasetv1.0", ['car', 'pedestrian', 'bicycle', 'motorcycle', 'bus', 'truck']
data_root, classes = "/datasets/nuScenes/Fulldatasetv1.0", ['car', 'pedestrian', 'bicycle']
# 1. 是否发布点云投影到图像的结果
...
# pc_filter_method, pc_filter_param = ['range'], {'limit_range': [-51.2, -51.2, -5.0, 51.2, 51.2, 3.0]}
# 5. 边界框过滤
box_filter_method, box_filter_param = [], None
# box_filter_method, box_filter_param = ['range'], {'limit_range': [-51.2, -51.2, -5.0, 51.2, 51.2, 3.0]}
# 6. 输入一个或多个评估文件
uniform_color, eval_pkls= False, []
# 7. 按照场景描述字段过滤点云帧
scene_filter_keys = None
# scene_filter_keys = ['night'] 
# scene_filter_keys = ['rain'] 
# scene_filter_keys = ['night', 'rain'] # and
# scene_filter_keys = ['reflections'] 
# scene_filter_keys = ['nature'] 

# -------------------------- config -------------------------- #

if __name__ == '__main__':
    rospy.init_node('pcdet_vis_nus_dataset_node', anonymous=True)

    pkl = rospy.get_param('pkl')
    start_idx = rospy.get_param('start_idx')

    infos = pd.read_pickle(pkl)
    dv = DatasetVisualizer(infos, classes[:3], default_cam='CAM_FRONT')
    ...
    while not rospy.is_shutdown():
        ...
        if scene_filter_keys is not None:
            mask = [(key in pc['desc'] ) for key in scene_filter_keys]
            if not all(mask):
